...
export class Environment {
    ...
    changeFoodLocation(vector) {
        this.foodPosition = (vector === undefined) ? new Vector(sketch.random(this.size.x * 0.95), sketch.random(this.size.y * 0.95)) : vector;
    }

    showFood(vector, sketch) {
        if (this.foodPosition === null)
            this.changeFoodLocation(vector, sketch);

        sketch.rect(this.foodPosition.x, this.foodPosition.y, this.foodScale, this.foodScale);
    }
